...
public class RuneBlockBakedModel implements IBakedModel {
    ...
    public List<BakedQuad> getQuads(@Nullable IBlockState state, @Nullable EnumFacing side, long rand) {
        ...
        switch( ext.getValue(BlockRune.FACING) ){
            case NORTH: return createQuad( new Vec3d(0,0,-1), new Vec3d(0,1,-1), new Vec3d(1,1,-1), new Vec3d(0,0,-1), sprite);
            case SOUTH: return createQuad( new Vec3d(0,0,1), new Vec3d(0,1,1), new Vec3d(1,1,1), new Vec3d(0,0,1), sprite);
            case WEST: return createQuad( new Vec3d(-1,0,0), new Vec3d(-1,1,0), new Vec3d(-1,1,1), new Vec3d(-1,0,1), sprite);
            case EAST: return createQuad( new Vec3d(1,0,0), new Vec3d(1,1,0), new Vec3d(1,1,1), new Vec3d(1,0,1), sprite);
            case UP: return createQuad( new Vec3d(0,1,0), new Vec3d(0,1,1), new Vec3d(1,1,1), new Vec3d(1,1,0), sprite);
    ...
    public static ResourceLocation getResLocFromRuneLoc(ResourceLocation matLoc, ResourceLocation chrLoc){
        if( matLoc == null || chrLoc == null )
            return MISSING_RUNE_TEXTURE;
        return new ResourceLocation( MortMagic.MODID, "rune_" + matLoc.getResourcePath() + "_" + chrLoc.getResourceDomain() );
    }
}
